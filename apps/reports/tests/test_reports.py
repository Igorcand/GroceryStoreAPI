...
def test_one_report_exists_should_succeed(client) -> None:
    report = Reports(product='Product', category='Category', 
...
def test_filtring_report_should_succeed(client) -> None:
    report1 = Reports(product='Product1', category='Category1', 
            ...
            payment='Money', data="2023-01-18")
    report1.save()
    report2 = Reports(product='Product2', category='Category2', 
            quantity_itens=3, stock=45, sale=45.70, 
            payment='Credit Card', data="2023-01-18")
    report2.save()
    response = client.post(path=reports_url, data={'data': "2023-01-18"}, content_type='application/json')
